...
# MAKE SURE you copy the ENTIRE output, including the "BASE64_IMAGES = {" and "}"
# Ensure no extra spaces or characters are introduced during copy-paste.
...
if st.button("✨ Reveal Your Character"):
    ...
    # --- DIAGNOSTIC PRINT (Will help if KeyError persists) ---
    st.info(f"Predicted character from model: '{character}'")
    # For debugging: print all keys in the dictionary if the error occurs
    # st.info(f"Keys available in BASE64_IMAGES: {list(BASE64_IMAGES.keys())}")
    ...
    if image_data_url:
        ...
        # Fallback if character's base64 data is not found (this means BASE64_IMAGES was not updated correctly)
        st.error(f"Image data for '{character}' not found in the embedded dictionary. "
                 "Please ensure the `BASE64_IMAGES` dictionary is correctly copied and contains this character.")
        st.markdown(
            f"""
            <div style="display: flex; justify-content: center; margin-bottom: 10px;">
